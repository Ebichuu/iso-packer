...
- CloudDrive2 API 用于观察、测试连接、封装前门禁和受控手动拉取
...
- 扫描 CD2 远程源目录，手动点选候选后调用 CD2 创建复制任务拉取到 `/watch`
...
6. 如果要在 Web 里从 CD2 远程源目录手动拉取原盘，再填写：
   ...
   `CD2 拉取目标目录` 留空时，会尝试用路径别名把 `/watch` 转换成 CD2 网盘路径。手动拉取只会对已配置源目录下的候选调用 CD2 `copy_file`，不会删除源文件，也不会取消你在 CD2 里手动创建的任务。
...
开启“CD2 手动拉取”后，它还会对配置源目录下的远程原盘候选创建 CD2 复制任务，把原盘拉到 `/watch`。它不会通过 API 直传 ISO，也不会自动删除、取消或接管你在 CD2 里手动创建的任务。
