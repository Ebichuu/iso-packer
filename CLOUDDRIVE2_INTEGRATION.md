...
- 真正的文件转移依旧走文件系统
- CD2 API 用于观察、测试连接、封装前门禁和受控手动拉取
...
再强调一次：这里不是 API 直传 ISO，也不是接管 CD2 任务。默认只做观察和封装前门禁；只有开启“CD2 手动拉取”后，才会对配置源目录下的候选创建 CD2 复制任务。

如果要从 CD2 远程源目录手动拉取原盘，再填写：
...
因为你现在的 ISO 输出位置已经固定，走文件系统移动更直接，项目也更轻。CD2 API 不用于把本地 ISO 直传网盘；它只负责观察、封装前门禁，以及在手动拉取开启时创建 CD2 自己的远程复制任务。
