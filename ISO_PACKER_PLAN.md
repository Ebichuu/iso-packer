...
当前已实现的 CD2 API 能力仅限只读观察、测试连接和封装前门禁；Webhook、目录刷新、远程目录扫描、手动 / 自动拉取都仍是后续规划。
...
8. 如果开启上传完成门禁，再读取 CD2 上传队列，确认云端上传完成后才把任务显示为最终完成。

事件入口与安全：

- 增加 CD2 Webhook / 事件通知入口，用于接收 CD2 或目录变化事件；默认关闭
- Webhook 入口复用当前 Web 服务端口 `15865`，规划路径为 `/api/cd2/webhook`
...
- 配置 CD2 云端源目录，例如 `/115/03-PT`，用于后续自动发现原盘
- 配置本地拉取目录，保持 `/watch`
...
- 允许调用 CD2 刷新指定目录，用于事件后刷新源目录和 ISO 目标目录状态
- 允许在自动化开启后创建下载 / 复制任务，把指定网盘原盘拉取到 `/watch`
...
1. 先把路径别名配置化：`/CloudNAS/CloudDrive` 和 `/115` 可在 Web 配置，并统一用于上传队列匹配、下载 / 复制门禁和目录观察。
2. 增加 CD2 事件入口：Webhook 鉴权、事件记录、去重、防抖，只触发复查，不直接改 ready。
3. 增加 `waiting_cd2_confirm`：事件触发后延迟确认，结合文件树稳定、临时文件识别、原盘结构检查、CD2 下载 / 复制任务状态。
4. 增加上传完成门禁：`cd2_wait_upload_complete` 开启时，`transfer_done` 之前进入 `waiting_cd2_upload`，等待 CD2 上传队列完成。
5. 增加远程目录只读扫描：Web 里能看到指定 CD2 云端源目录下的 BDMV / VIDEO_TS 候选。
6. 增加手动拉取：用户点选候选后，由 iso-packer 调用 CD2 创建下载 / 复制任务。
7. 增加全自动拉取：独立开关开启时，自动发现、自动拉取、自动封装、自动转移。

计划新增配置：

- `cd2_webhook_enabled`、`cd2_webhook_secret`、`cd2_event_source`
...
- 上传完成门禁不会把“本地已转移”误显示成“云端已上传”
- 自动拉取只跟踪自己创建的 CD2 任务，不接管外部手动任务
...
- 本轮计划中的基础项已经落地：基础结构拆分、独立自动化测试、CD2 状态增强、失败原因展示、重封装按钮状态增强
