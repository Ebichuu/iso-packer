...
- 主线版本：`ebichu/iso-packer:latest` / `ebichu/iso-packer:1.1.9.9`
- 使用方式：单 VPS、单容器、个人自用
- 固定流程：
...
- 已新增远程候选批量选择和批量拉取：在当前筛选 / 搜索结果内勾选候选后，可顺序创建 CD2 拉取任务；成功项自动取消选中，失败项保留便于重试。
- 已新增批量拉取失败重试反馈：批量失败后显示持久结果条，失败项保留选中，按钮切换为“重试选中”。
- 已新增远程候选批量清除记录：可对当前选中的失败 / 已结束候选批量清除拉取记录，成功项自动取消选中，失败项保留便于重试。
- 已新增失败 / 警告操作建议：空间不足、封装失败、校验失败、CD2 转移失败、任务异常、源文件删除失败会在任务列表里显示下一步排查方向。
...
## 参考 SA/Symedia 后的优化候选

`D:\Projects\symedia-local` 更像 SA/Symedia 的本地重建和前端资源工程，不是完整生产后端；其中最值得参考的是归档任务、历史记录、日志和插件配置的交互方式，而不是 TMDB、Emby、洗版、STRM 或通用插件市场。

推荐吸收：

- 任务详情和按任务日志：参考 SA 的“任务日志”入口，把当前全局事件日志补一层按任务筛选，方便定位某个 ISO 从发现、等待、封装、转移到上传观察的全过程。
- 历史记录筛选和批量管理：参考 SA 的历史页，给 iso-packer 增加状态筛选、搜索、分页、批量删除历史记录；源文件 / 目标文件删除保持谨慎，默认不开放危险操作。
- 任务暂停 / 停止入口：先做“停止排队、暂停扫描、暂停自动拉取”，避免正在生成 ISO 时被误打断；是否允许终止 `genisoimage` 作为单独实验项。
- 监控策略表达优化：参考 SA 的“事件通知 / 轮询模式”，把 iso-packer 的 CD2 webhook、轮询兜底、扫描间隔和确认延迟整理成更容易理解的“监控策略”设置区。
- 失败处理增强：参考 SA 的失败目录概念，先增加“失败任务保留 / 重试 / 清理记录”的可见操作；是否移动失败目录需要谨慎评估，避免对 BDMV 大目录做慢速跨盘移动。
- 影片库存 / 版本视图：记录已经封装和转移过的影片，展示片名、年份、发布组、清晰度、视频编码、音频规格、杜比视界 / HDR 信息、ISO 路径、封装时间和上传状态；同一影片可以显示多个不同发布组 / 版本，方便知道自己已经入了哪些盘。

暂缓：

- SSE 实时推送：SA 用 SSE 推任务状态和日志，体验更实时；iso-packer 当前轮询已经够用，可在页面轮询压力或延迟明显时再切。
- 配置模板 / 复制任务：SA 支持复制任务和规则；iso-packer 当前个人使用配置较少，暂不作为优先项。
- 本地 watch 忽略关键词 / 后缀：SA 有 `sample`、`trailer`、`bonus`、`.tmp` 等忽略规则；iso-packer 处理原盘目录，误跳风险比收益更高，后续只做可选高级项。

明确不做：

- 不引入 SA 的 TMDB、Emby、权重洗版、重命名入库、STRM、AI 识别。
- 不做自动洗版和自动替换决策；后续的“多版本 / 发布组”只作为库存展示、筛选和人工判断依据。
- 不做通用插件系统或插件市场。
- 不把 iso-packer 变成完整媒体库管理器，主线仍保持“原盘拉取 -> ISO 封装 -> CD2 上传观察”的个人自动化工具。
...
- 手动“重新封装”同样遵守未完成下载保护
- 重新封装前会检查 `/output` 同名 ISO：中断残留自动清理，疑似完整成品保留并提示手动处理；上传目录同名 ISO 不自动改名或加时间尾巴
- 前端任务状态刷新改为串行轮询，并给 `/api/status` 增加 15 秒前端超时，避免 CD2 队列慢响应时状态请求堆叠或旧响应覆盖新画面
...
- 已初步拆分 `fetch_cd2_uploads()` 的缓存、基础状态、上传条目解析和成功状态缓存逻辑
- 已初步拆分 `transfer_iso_to_mount()` 的目标路径准备、分块复制、完成校验和 CD2 刷新逻辑
- 已初步拆分 `process_item()` 的非 ISO 清理、空间不足、任务启动、封装 / 校验 / 转移失败、源文件删除和成功收尾状态逻辑
...
- 新增本地集成验证脚本 `scripts/integration_smoke.py` 和说明 `INTEGRATION_TESTING.md`，用于真实 `genisoimage` / `xorriso` 工具链 smoke test
- 新增 CD2 API 只读探针 `scripts/cd2_readonly_probe.py`，用于真实环境下只读验证上传 / 下载 / 复制队列和路径匹配，不创建 CD2 任务

## 当前仍保留为后续优化

这些不影响当前个人使用和部署，不作为本轮阻塞项：

- `process_item()`、`fetch_cd2_uploads()` 和 `transfer_iso_to_mount()` 后续只做小步整理
- 继续补充真实 CD2 环境下的人工验收记录，例如在 VPS 临时容器里跑只读探针后的输出结论
- 继续细化 CD2 自动拉取的真实使用体验，例如路径匹配解释和失败提示等

## 本轮检查结论

- 当前核心链路仍按个人 VPS + Docker + CD2 挂载方案运行
- 没有重新引入 TMDB、qB 深度集成、Agent 或分布式能力
- 本轮计划中的基础项已经落地：基础结构拆分、独立自动化测试、CD2 状态增强、失败原因展示、重封装按钮状态增强、任务列表行内进度展示、CD2 Webhook 基础事件入口、`waiting_cd2_confirm` 延迟确认门禁、CD2 目录刷新、远程候选扫描、CD2 手动拉取、CD2 自动拉取基础能力、远程候选拉取状态展示、远程候选清除记录后重拉、CD2 缓存状态显示、远程候选流程状态筛选、任务失败分类、远程候选搜索、远程候选批量选择和批量拉取、批量拉取失败重试反馈、远程候选批量清除记录、失败 / 警告操作建议
