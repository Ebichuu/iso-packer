...
- 主线版本：`ebichu/iso-packer:latest` / `ebichu/iso-packer:1.1.3`
- 使用方式：单 VPS、单容器、个人自用
- 固定流程：
...
- 本轮计划中的基础项已经落地：基础结构拆分、独立自动化测试、CD2 状态增强、失败原因展示、重封装按钮状态增强、任务列表行内进度展示、CD2 Webhook 基础事件入口、`waiting_cd2_confirm` 延迟确认门禁、CD2 目录刷新、远程候选扫描、CD2 手动拉取、CD2 自动拉取基础能力、远程候选拉取状态展示、远程候选清除记录后重拉、CD2 缓存状态显示、远程候选流程状态筛选
