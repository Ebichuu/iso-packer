...
最终目标是把 iso-packer 从“只监控本地 `/watch`”扩展为类似 MoviePilot2 / Symedia 的“网盘监控目录”模式：用户在 Web 里配置一个或多个 CD2 网盘源目录，iso-packer 只观察这些目录；发现完整 BDMV / VIDEO_TS 原盘候选后，由 iso-packer 调用 CD2 把原盘拉取到 `/watch`，再交给现有本地封装、转移和上传观察流程。这条自动化按阶段开放，每一步都可以单独关闭：

1. 远程目录观察：只读展示 CD2 云端监控目录里的原盘候选。
...
- 把 `cd2_remote_source_dirs` 在界面上明确为“CD2 远程监控目录”，每行一个允许扫描的网盘源目录。
