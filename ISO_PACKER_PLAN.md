...
- 增加监控策略：递归深度、最小体积、全局同时拉取任务数；每轮创建任务数、包含 / 排除关键词已支持配置。
